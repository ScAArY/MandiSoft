...
Class account extends CI_Model {
    ...
    function get_ledger_sales($a,$b=20,$c=20){
                $query = $query = $this->db->query(
                        "SELECT * FROM (SELECT s.`sold_to`,a.`acc_name`, s.sale_id, s.quantity,s.rebate,qt.qnt_sign,s.rate, s.rebate AS sale_rebate,s.item_lot,s.datetime,s.sale_date AS sale_date, dab.old_balance, dab.new_balance FROM `item_sale` as s JOIN `accounts` a ON ( s.`sold_to` = a.`acc_id` ) LEFT JOIN `datewise_account_balance` dab ON (dab.sale_id = s.sale_id) JOIN `quantity_types` as qt ON(qt.qnt_id = s.quantity_type) WHERE a.ledger_number=$a ORDER BY s.`datetime` DESC LIMIT $b ) as f ORDER BY f.datetime ASC LIMIT $c"
                );
                return $query->result();
    }
